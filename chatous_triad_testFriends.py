...
def test_on_friends(test_friend,X,Y):
#find feature for test_friend
	x_vec=[] #friend feature
	y_vec=[]
	total=0
	frac_triad=[0]*8
	for row in test_friend:
		node1=test_friend[0]
		node2=test_friend[1]
		...
		if node1==node2:
			continue
		if not graph.IsEdge(GetNI(node1),GetNI(node2)):
			continue
		intersect=find_mutual_triad_node(GetNI(node1),GetNI( node2), graph)
...
#	************************cross validation
	...
	X_test=x_vec
	y_test=y_vec
	CV=sm.GLM(Y,X,family=sm.families.Binomial())
	CV_fitted=CV.fit()
...
polarity={}
#read graph
inputFriends=open('pos_neg_gold.txt')
friend={}
	...
	for word in line.split('\t' or '\n'):
		...
		if count==0:
			...
			if (node1,node2) not in friend:				
				...
				test_friend.append([node1,node2])
				frac10=frac10+1

#print test_friend			
#print "added friends: ", fcount		
inputF.close()
inputFriends.close()
#number of triads
...
print "number of edges to be predicted: ", len(test_friend)
#triad_training(graph,edge_sign)
polarity={}
#polarity_training(polarity,graph,edge_sign)
x_vec=load(open("triad_feature_X","rb"))
y_vec=load(open("triad_feature_Y","rb"))
test_on_friends(test_friend,x_vec,y_vec)
